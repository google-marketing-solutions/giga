...
const getAdsClient = (customerIdOverride?: string) => {
  const developerToken =
    properties.DEVELOPER_TOKEN || getEnvVar('GOOGLE_ADS_DEVELOPER_TOKEN', '');
  ...
  const customerId =
    customerIdOverride ||
    properties.ADS_ACCOUNT_ID ||
    getEnvVar('GOOGLE_ADS_CUSTOMER_ID', '');
  ...
  const schema = {
    ...
    properties: {
      id: {
        type: 'string',
        description:
          'The Google Ads language criterion ID (e.g., 1000 for English)',
      },
      name: {
        type: 'string',
        description: 'The standard name of the language (e.g., English)',
      },
    },
    required: ['id', 'name'],
  };
...
export async function getLocationId(
  ...
  try {
    ...
    const responseData = suggestionsResponse as unknown as Record<
      string,
      unknown
    >;
    ...
    if (geoTarget) {
      return {
        ...
        name:
          ...
          geoTarget.canonical_name ||
          '',
      };
    }
  } catch (error) {
    console.error(
      'Failed to get location suggestions from Google Ads API',
      error,
    );
  }
  ...
  const schema = {
    ...
    properties: {
      id: {
        type: 'string',
        description:
          'The Google Ads location criterion ID (e.g., 2840 for United States)',
      },
      name: {
        type: 'string',
        description: 'The standard name of the location (e.g., United States)',
      },
    },
    required: ['id', 'name'],
  };
...
export async function getLocationById(id: string) {
  try {
    ...
    if (result) {
      return {
        id: String(result.id),
        name: result.name || result.canonical_name,
      };
    }
  } catch (e) {
...
export async function getScriptPropertiesConfiguration(
  localConfig?: Record<string, string>,
) {
  if (localConfig) {
    if (localConfig.ADS_ACCOUNT_ID)
      properties.ADS_ACCOUNT_ID = localConfig.ADS_ACCOUNT_ID;
    if (localConfig.DEVELOPER_TOKEN)
      properties.DEVELOPER_TOKEN = localConfig.DEVELOPER_TOKEN;
    if (localConfig.GEMINI_API_KEY)
      properties.GEMINI_API_KEY = localConfig.GEMINI_API_KEY;
  }
...
export async function generateKeywordIdeas(
  ...
  const keywordsArray = Array.isArray(seedKeywords)
    ? seedKeywords
    : seedKeywords
        .split(' ,')
        .map(s => s.trim())
        .filter(Boolean);
  const result = await getKeywordIdeasFromSeed(
    keywordsArray,
...
export async function generateTrendsKeywords(
  seedKeywords: string | string[],
  prompt: string,
  config: Record<string, unknown>,
) {
  const seedsStr = Array.isArray(seedKeywords)
    ? seedKeywords.join(' , ')
    : seedKeywords;
...
export async function getClusters(
  ...
  const keywords = Object.keys(ideas)
    .map(k => k.toLowerCase())
    .join(', ');
  ...
  const result = (await geminiRequest(
    ...
    responseSchema,
    false,
  )) as {topic: string; keywords: string[]}[];

  const lowerIdeas = Object.keys(ideas).reduce(
    (acc, key) => {
  ...
  return result.map(cluster => {
    cluster.keywords = cluster.keywords.filter(
      kw => kw.toLowerCase() in lowerIdeas,
    );

    // Aggregating metrics for the cluster
    const clusterVols = cluster.keywords.map(
      kw => lowerIdeas[kw.toLowerCase()],
    );
    ...
    return {
      ...
      searchVolume:
        aggregatedHistory.reduce((a, b) => a + b, 0) / aggregatedHistory.length,
...
export async function getInsightsChatResponse(
  ...
  const result = (await geminiRequest(
    ...
    finalPrompt,
    responseSchema,
    config?.enableGoogleSearch as boolean,
  )) as Record<string, unknown>;

  if (
    result.image_prompts &&
    Array.isArray(result.image_prompts) &&
    ...
    try {
      ...
      const imageModel =
        (config?.imageModelId as string) || 'gemini-2.5-flash-image';
...
export async function getInsights(
  ...
  const prompt = getInsightsPrompt(
    relevantIdeas,
    Array.isArray(seedKeywords) ? seedKeywords : seedKeywords.split(' , '),
    metricName,
    language,
    specificQuestion,
  );
...
export async function getTopPerformingAdsAndKeywords(
  cid: string,
  topN: number,
  metric = 'clicks',
) {
...
export async function getCampaigns(
  ...
  const sortedIdeas = relevantIdeasList
    .sort((a, b) => b[1] - a[1])
    .slice(0, 100);
  const keywordsStr = sortedIdeas
    .map(row => `${row[0]} (${(row[1] * 100).toFixed(0)}% ${growthMetric})`)
    .join(', ');
...
export async function createAdSuggestion(
  prompt: string,
  userKeywords: string[],
  config: Record<string, unknown>,
) {
